...
**🩺 Which references resolve changed in `0.0.17`, and it changes how some already-published files
decode.** Through `0.0.16` the reader recognised exactly three: `005010X222A2`, `005010X223A3` and
...
published errata. **If you read 837 files on `0.0.16` or earlier, re-check any routing you drove off
`submission.variant`, and any predicate you wrote on `X12_837_UNKNOWN_VARIANT` or
`X12_837_AMBIGUOUS_VARIANT`:** on a file whose ST-03 is now recognised, the variant can differ and
neither code fires any more. The set is a list of cited identifiers, never a pattern: a reference
