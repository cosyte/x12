...
| A claim's `serviceLines` is empty but the file has `LX` segments | An `X12_837_SERVICE_LINE_DROPPED` warning at each such `LX`: it opened no Loop 2400, so the line reached no claim at all. Either no `CLM` was open at that point in the transaction, or the submission's variant never resolved to `P` / `I` / `D` (which also raises `X12_837_UNKNOWN_VARIANT`). | Do **not** read the empty `serviceLines` as "the claim had no service lines". Nothing was fabricated to stand in and no claim was synthesized; the segments are verbatim on `tx.segments[…].raw`. If the cause is the variant, re-read with the matching `type` option. This is a different loss from the row above, where the line **is** on the model and only its service segment went unread. |
