...
| An amount or quantity reads `0` and the payer says otherwise | An `X12_UNPARSEABLE_DECIMAL` warning at that element: the sender put bytes in a decimal element that are not an X12 R-type decimal (a thousands separator, a currency symbol, `N/A`), so nothing decoded and the slot fell back to `X12Decimal.ZERO` | Treat that `0` as unread, not as zero. The warning's `position.elementIndex` names the element; the verbatim bytes are on `tx.segments[…].raw`. An **absent** element does not warn, so a `0` with no warning really is a zero the sender sent. See [Decimal-exact money](./spec-notes-money). |
