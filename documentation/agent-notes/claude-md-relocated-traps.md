# Traps, relocated from CLAUDE.md

CLAUDE.md keeps the `## Traps` heading and a pointer at this file; everything below stood under that heading there and is moved here
unchanged, to bring the agent doc back inside the context ceiling its readers load it whole against. Nothing was dropped and no heading
was renamed. Paths read as CLAUDE.md wrote them, so `agent-notes/<slug>.md` means `documentation/agent-notes/<slug>.md`.
