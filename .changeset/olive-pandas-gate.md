...
The documentation a consumer reads, and the JSDoc their editor renders, no longer carry internal
project bookkeeping, and a gate now keeps it that way. Documentation and tooling only: the diff is
comment-only in `src/`, and `dist/index.mjs` / `dist/index.cjs` are byte-identical before and after,
so no behaviour moved.

What a consumer receives changes in two places, and deliberately not in a third: source string
literals are outside this gate, and five bundled code-list snapshots still carry build-order framing
in an exported `note` field, which is a runtime value with a different carrier and review path from
a comment. That reproduces on the base and is filed rather than folded in here.

`KNOWN-LIMITATIONS.md` and the documentation pages
...
cannot open. The built declaration file now reads zero on all six rules, and contains no occurrence
of the word "phase" at all, which is the stronger statement: six lines survived the first sweep
because two used the plural, which the rule cannot see, and four ended in punctuation.
...
reports. Read that precisely, because a positive sample is disjunctive: it proves each rule still
matches SOMETHING it bans, plus five named spellings asserted individually, and it proves each rule
still lets through the 005010 segment-field references this package's documentation exists to
provide. Most individual rule alternatives are asserted by nothing, which is measured and disclosed
in the script rather than left to be discovered.
