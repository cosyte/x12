...
One surface is named rather than fixed here. Nine `build*` functions interpolate an over-long control number into the thrown message verbatim, and `build834` / `buildTA1` do the same with an unrecognized maintenance type or note code, so a 120,000-byte caller value gives a 120,190-byte `Error.message`. The README, the troubleshooting page and `KNOWN-LIMITATIONS.md` now name that surface and say to log `err.code` from a builder. Bounding the builders is a separate change.
