...
🩺 `implementationConventionReference` is POST-`?`-unescape in every typed reader that publishes the
field (`X12-ST03-READ-NOT-RELEASE-AWARE`). A behaviour change on any document whose `ST-03` carries a
release escape.
