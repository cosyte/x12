...
element comes back a prefix and every element after it is displaced by one.
...
by index, so `{ strict: true }` escalates on it rather than on a displaced-value warning.
...
005010-conformant either way and 005010 does not say which reading to take, so `isa.elements` is left
