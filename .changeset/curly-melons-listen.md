...
**It is additive and nothing moved onto it.** `X12_837_SERVICE_LINE_NOT_DECODED`, `X12_837_SERVICE_SEGMENT_WITHOUT_LX` and `X12_837_SERVICE_LINE_DROPPED` fire on exactly the documents they fired on before, in the same positions, pinned by committed tests that assert the whole warning channel with the new code filtered out. **No consumer predicate written against any existing code changes meaning.**

**What it does not close.** A foreign or duplicate `SVx` arriving inside a Loop 2400 the resolved variant already decoded is still read into nothing in silence at the segment level, and on a document where that segment is the only thing contradicting the resolution this code is the sole report on the channel. Narrowing the fallback, and warning at that segment, are each their own slice.
