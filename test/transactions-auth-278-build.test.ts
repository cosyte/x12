...
describe("X12-CONTROL-NUMBER-GUARD-NOT-TYPE-CHECKED: build278", () => {
  ...
  it("🩺 refuses a non-string interchangeControlNumber (ISA-13 / IEA-02)", () => {
    expect(() =>
      ...
      /build278: interchangeControlNumber must be a string, but received an array\. ISA-13 \/ IEA-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string groupControlNumber (GS-06 / GE-02)", () => {
    expect(() =>
      ...
      /build278: groupControlNumber must be a string, but received an array\. GS-06 \/ GE-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string transactionSetControlNumber (ST-02 / SE-02)", () => {
    expect(() =>
      ...
      /build278: transactionSetControlNumber must be a string, but received an array\. ST-02 \/ SE-02 is a required control number/,
    );
  });
...
describe("build278Request - X12-EMIT-DEGENERATE-RELEASE-DELIMITER", () => {
  it('🩺 refuses a delimiter set whose segment terminator is the release character "?"', () => {
    ...
    // A released byte inside any value would terminate the segment early.
    expect(() =>
      build278Request({
        ...CANONICAL_SPEC,
        envelope: { ...ENVELOPE, segmentTerminator: "?" },
      }),
    ).toThrow(
      'build278: "?" is the X12 release character and cannot also be the segment terminator.',
    );
  });

  it("the same spec on the conventional set still builds (the red control)", () => {
    expect(build278Request(CANONICAL_SPEC).warnings).toEqual([]);
  });
});
