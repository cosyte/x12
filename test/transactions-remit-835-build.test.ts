...
describe("build835 - an empty envelope control number is refused", () => {
  ...
  it("🩺 refuses an empty interchangeControlNumber (ISA-13 / IEA-02)", () => {
    expect(() =>
      build835({ ...BALANCED_SPEC, envelope: { ...ENVELOPE, interchangeControlNumber: "" } }),
    ).toThrow(
      /build835: interchangeControlNumber is empty\. ISA-13 \/ IEA-02 is a required control number/,
    );
  });

  it("refuses an empty groupControlNumber (GS-06 / GE-02)", () => {
    expect(() =>
      build835({ ...BALANCED_SPEC, envelope: { ...ENVELOPE, groupControlNumber: "" } }),
    ).toThrow(
      /build835: groupControlNumber is empty\. GS-06 \/ GE-02 is a required control number/,
    );
  });

  it("refuses an empty transactionSetControlNumber (ST-02 / SE-02)", () => {
    expect(() =>
      build835({ ...BALANCED_SPEC, envelope: { ...ENVELOPE, transactionSetControlNumber: "" } }),
    ).toThrow(
      /build835: transactionSetControlNumber is empty\. ST-02 \/ SE-02 is a required control number/,
    );
  });
  ...
  it("still builds the unmodified spec, and still zero-pads a SHORT control number", () => {
    ...
    expect(build835(BALANCED_SPEC).warnings).toHaveLength(0);
    const short = build835({
      ...BALANCED_SPEC,
      envelope: { ...ENVELOPE, interchangeControlNumber: "1" },
    });
...
describe("X12-CONTROL-NUMBER-GUARD-NOT-TYPE-CHECKED: build835", () => {
  ...
  it("🩺 refuses a non-string interchangeControlNumber (ISA-13 / IEA-02)", () => {
    expect(() =>
      ...
      /build835: interchangeControlNumber must be a string, but received an array\. ISA-13 \/ IEA-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string groupControlNumber (GS-06 / GE-02)", () => {
    expect(() =>
      ...
      /build835: groupControlNumber must be a string, but received an array\. GS-06 \/ GE-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string transactionSetControlNumber (ST-02 / SE-02)", () => {
    expect(() =>
      ...
      /build835: transactionSetControlNumber must be a string, but received an array\. ST-02 \/ SE-02 is a required control number/,
    );
  });
...
describe("build835 - X12-EMIT-DEGENERATE-RELEASE-DELIMITER", () => {
  it('🩺 refuses a delimiter set whose repetition separator is the release character "?"', () => {
    ...
    // A repetition joined with `?` is emitted as an escape sequence.
    expect(() =>
      build835({
        ...BALANCED_SPEC,
        envelope: { ...ENVELOPE, repetitionSeparator: "?" },
      }),
    ).toThrow(
      'build835: "?" is the X12 release character and cannot also be the repetition separator.',
    );
  });

  it("the same spec on the conventional set still builds (the red control)", () => {
    expect(build835(BALANCED_SPEC).warnings).toEqual([]);
  });
});
