...
describe("X12-CONTROL-NUMBER-GUARD-NOT-TYPE-CHECKED: build271", () => {
  ...
  it("🩺 refuses a non-string interchangeControlNumber (ISA-13 / IEA-02)", () => {
    expect(() =>
      ...
      /build271: interchangeControlNumber must be a string, but received an array\. ISA-13 \/ IEA-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string groupControlNumber (GS-06 / GE-02)", () => {
    expect(() =>
      ...
      /build271: groupControlNumber must be a string, but received an array\. GS-06 \/ GE-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string transactionSetControlNumber (ST-02 / SE-02)", () => {
    expect(() =>
      ...
      /build271: transactionSetControlNumber must be a string, but received an array\. ST-02 \/ SE-02 is a required control number/,
    );
  });
...
describe("build271 - X12-EMIT-DEGENERATE-RELEASE-DELIMITER", () => {
  it('🩺 refuses a delimiter set whose repetition separator is the release character "?"', () => {
    ...
    // EB-03 is a REPEATING element joined with this separator, so on `?` the two
    // service type codes emitted as one escape and read back as one code.
    expect(() =>
      build271({
        ...CANONICAL_SPEC,
      ...
      'build271: "?" is the X12 release character and cannot also be the repetition separator.',
    );
  });

  it("the same spec on the conventional set still builds (the red control)", () => {
    expect(build271(CANONICAL_SPEC).warnings).toEqual([]);
  });
});
