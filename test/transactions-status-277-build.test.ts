...
describe("build277 - structural refusals", () => {
  ...
  it("bounds its refusal message against a 120,000-character control number", () => {
    ...
    try {
      build277({ ...CANONICAL_SPEC, envelope: { ...ENVELOPE, interchangeControlNumber: huge } });
      throw new Error("expected build277 to refuse an over-long control number");
    } catch (err) {
      expect(err).toBeInstanceOf(ClaimStatus277BuildError);
...
describe("build277 - an empty envelope control number is refused", () => {
  ...
  it("🩺 refuses an empty interchangeControlNumber (ISA-13 / IEA-02)", () => {
    expect(() =>
      build277({ ...CANONICAL_SPEC, envelope: { ...ENVELOPE, interchangeControlNumber: "" } }),
    ).toThrow(
      /build277: interchangeControlNumber is empty\. ISA-13 \/ IEA-02 is a required control number/,
    );
  });

  it("refuses an empty groupControlNumber (GS-06 / GE-02)", () => {
    expect(() =>
      build277({ ...CANONICAL_SPEC, envelope: { ...ENVELOPE, groupControlNumber: "" } }),
    ).toThrow(
      /build277: groupControlNumber is empty\. GS-06 \/ GE-02 is a required control number/,
    );
  });

  it("refuses an empty transactionSetControlNumber (ST-02 / SE-02)", () => {
    expect(() =>
      build277({ ...CANONICAL_SPEC, envelope: { ...ENVELOPE, transactionSetControlNumber: "" } }),
    ).toThrow(
      /build277: transactionSetControlNumber is empty\. ST-02 \/ SE-02 is a required control number/,
    );
  });
  ...
  it("still builds the unmodified spec, and still zero-pads a SHORT control number", () => {
    ...
    expect(build277(CANONICAL_SPEC).warnings).toHaveLength(0);
    const short = build277({
      ...CANONICAL_SPEC,
      envelope: { ...ENVELOPE, interchangeControlNumber: "1" },
    });
...
describe("X12-CONTROL-NUMBER-GUARD-NOT-TYPE-CHECKED: build277", () => {
  ...
  it("🩺 refuses a non-string interchangeControlNumber (ISA-13 / IEA-02)", () => {
    expect(() =>
      ...
      /build277: interchangeControlNumber must be a string, but received an array\. ISA-13 \/ IEA-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string groupControlNumber (GS-06 / GE-02)", () => {
    expect(() =>
      ...
      /build277: groupControlNumber must be a string, but received an array\. GS-06 \/ GE-02 is a required control number/,
    );
  });
  ...
  it("refuses a non-string transactionSetControlNumber (ST-02 / SE-02)", () => {
    expect(() =>
      ...
      /build277: transactionSetControlNumber must be a string, but received an array\. ST-02 \/ SE-02 is a required control number/,
    );
  });
...
describe("build277 - X12-EMIT-DEGENERATE-RELEASE-DELIMITER", () => {
  it('🩺 refuses a delimiter set whose component separator is the release character "?"', () => {
    ...
    // Every composite this builder joins would be emitted as an escape sequence.
    expect(() =>
      build277({
        ...CANONICAL_SPEC,
      ...
      'build277: "?" is the X12 release character and cannot also be the component separator.',
    );
  });

  it("the same spec on the conventional set still builds (the red control)", () => {
    expect(build277(CANONICAL_SPEC).warnings).toEqual([]);
  });
});
