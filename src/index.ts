/**
 * Public entry point for the `@cosyte/x12` package. It lands the envelope
 * decoder (ISA / GS / GE / IEA + delimiter detection), segment/element/
 * composite decode, the warning-code registry, and the public
 * `defineLoopSpec`. Per-transaction helpers
 * (`get835`, `get837Claims`, ...) sit on top of it.
 */

/**
 * Library version string, synced with `package.json#version` by the
 * release's version step. Exported now so consumers (and the
 * type-check pipeline) have at least one symbol to resolve through the
 * `exports` map.
 *
 * @example
 * ```ts
 * import { VERSION } from "@cosyte/x12";
 * console.log(VERSION);
 * ```
 */
export const VERSION: string = "0.0.18";

// The package's conformance declaration: which implementation guide each
// reader and builder implements, which identifiers 45 CFR 162.920 names for
// that transaction, and which directions are actually implemented. Static
// data; the typed readers derive the guides they check a declaration against
// from it, and it changes no emit behaviour.
export { X12_TR3_CONFORMANCE } from "./conformance/index.js";
export type { X12Tr3Adoption, X12Tr3Conformance, X12Tr3Direction } from "./conformance/index.js";

// Phase 1 - envelope parser surface.
export { parseX12 } from "./parser/index.js";
export { detectDelimiters, DELIMITER_POSITIONS, ISA_MIN_LENGTH } from "./parser/delimiters.js";
export { FATAL_CODES, X12ParseError } from "./parser/errors.js";
export type { X12FatalCode } from "./parser/errors.js";
export {
  AAA_LEVEL_CONTEXTS,
  ALL_WARNING_MESSAGES,
  BALANCE_INVARIANTS,
  CONTROL_NUMBER_PAIRS,
  REQUIRED_LOOPS,
  UNEXPECTED_SEGMENT_CONTEXTS,
  WARNING_CODES,
  aaaLoopUnidentified,
  aaaRejectReasonAbsent,
  aaaSegmentMalformed,
  aaaUnknownCode,
  ambiguous837Variant,
  amountRowDropped,
  attachmentAbsent,
  balanceNotEvaluable,
  binaryDataTruncated,
  binaryLengthInvalid,
  binaryLengthMismatch,
  binaryLengthUnverifiable,
  controlNumberMismatch,
  danglingReleaseChar,
  dateRowDropped,
  duplicateHierarchyId,
  entitySegmentDiscardedAfterLx,
  groupCountMismatch,
  guideNotDeclared,
  guideNotImplemented,
  hierarchyCycle,
  hlParentLevelInvalid,
  hlParentMismatch,
  interSegmentLineBreak,
  isaExtraElementSeparator,
  levelDetached,
  missingGe,
  missingIea,
  missingRequiredLoop,
  missingSe,
  nonConventionalDelimiter,
  payToAddressRepeated,
  pre005010,
  remitBalanceMismatch,
  rfaiHeaderAbsent,
  rfaiLevelAbsent,
  rfaiRequestAbsent,
  segmentCountMismatch,
  serviceLineDropped,
  serviceLineNotDecoded,
  serviceSegmentRepeated,
  serviceSegmentWithoutLx,
  statedAmountDiscarded,
  statusInquiryDateRowDropped,
  statusInquiryDuplicateHierarchyId,
  statusInquiryHierarchyCycle,
  statusInquiryLevelDetached,
  statusInquiryReferenceRowDropped,
  trailingGarbage,
  transactionCountMismatch,
  unexpectedSegment,
  unknown837Variant,
  unknownCarc,
  unknownClaimStatus,
  unknownClaimStatusCategory,
  unknownHiQualifier,
  unknownMaintenanceType,
  unknownRarc,
  unparseableDecimal,
} from "./parser/warnings.js";
export type {
  X12AaaLevelContext,
  X12BalanceInvariant,
  X12ControlNumberPair,
  X12ParseWarning,
  X12RequiredLoop,
  X12UnexpectedSegmentContext,
  X12WarningCode,
} from "./parser/warnings.js";
export type {
  Delimiters,
  GeSegment,
  GsSegment,
  IeaSegment,
  IsaSegment,
  OnWarningCallback,
  Ta1Segment,
  X12FunctionalGroup,
  X12Interchange,
  X12OrphanAnchor,
  X12OrphanAnchorKind,
  X12OrphanSegment,
  X12ParseOptions,
  X12Position,
  X12TransactionSet,
} from "./parser/types.js";

// Phase 2 - segment / element / composite / repetition decode surface.
export { escapeRelease, RELEASE_CHAR, unescapeRelease } from "./parser/release.js";
export {
  NON_SPEC_SEGMENT_ID,
  collectElementValues,
  componentOptional,
  decodeSegment,
  elementDecimal,
  elementDecimalOrZero,
  elementOptional,
  elementValue,
  getAllSegmentValues,
  getSegmentValue,
  readElementDecimal,
} from "./parser/segment.js";
export type {
  X12DecimalRead,
  X12DecimalStatus,
  X12DecimalWarningSink,
  X12Segment,
} from "./parser/segment.js";

// Phase 2 - loop-spec authoring surface (dogfooded by built-in transaction
// specs in Phases 3+).
export { defineLoopSpec, LoopSpecDefinitionError } from "./loops/define.js";
export type { DefineLoopSpecInput } from "./loops/define.js";
export type { LoopMax, LoopSegmentSpec, LoopSpec, LoopUsage } from "./loops/types.js";

// Phase 4 - money + bundled code-list snapshots used by 835.
export { X12Decimal } from "./decimal.js";
export {
  AAA_FOLLOW_UP_ACTION_CODES,
  AAA_REJECT_REASON_CODES,
  CARC,
  CLAIM_ADJUSTMENT_GROUP_CODES,
  CLAIM_STATUS_CATEGORY_CODES,
  CLAIM_STATUS_CODES,
  CLP_STATUS,
  CODE_VALIDITY,
  CODE_VALIDITY_REASONS,
  MAINTENANCE_TYPE_CODES,
  RARC,
  HI_QUALIFIERS,
  SERVICE_TYPE_CODES,
  X12_CODE_LIST_ERROR_CODES,
  X12CodeListError,
  aaaProvenanceIsComplete,
  bundleAaaCodes,
  checkCarcValidity,
  checkRarcValidity,
  codeListRedistributionIsPermitted,
  isClaimAdjustmentGroupCode,
  isDiagnosisQualifier,
  isProcedureQualifier,
  lookupAaaFollowUpAction,
  lookupAaaRejectReason,
  lookupCarc,
  lookupClaimStatus,
  lookupClaimStatusCategory,
  lookupClpStatus,
  lookupMaintenanceType,
  lookupRarc,
  lookupServiceType,
  resolveHiQualifier,
  type AaaCodeListMeta,
  type AaaCodeListSnapshot,
  type ClaimAdjustmentGroupCode,
  type CodeListCompleteness,
  type CodeListEntry,
  type CodeListEntryDates,
  type CodeListMeta,
  type CodeListRedistribution,
  type CodeListRedistributionStatus,
  type CodeListSnapshot,
  type CodeValidity,
  type CodeValidityReason,
  type CodeValidityResult,
  type DatedCodeListMeta,
  type DatedCodeListSnapshot,
  type X12CodeListErrorCode,
  type X12HiCategory,
  type X12HiCodeSystem,
  type X12HiQualifier,
} from "./code-lists/index.js";

// Phase 4 - 835 Healthcare Claim Payment/Advice (ERA) surface (TR3 005010X221A1).
export {
  CORE_BUSINESS_SCENARIOS,
  CORE_CODE_COMBINATION_ERROR_CODES,
  CORE_CODE_COMBINATION_OUTCOMES,
  CORE_CODE_COMBINATION_UNEVALUATED_REASONS,
  CoreCodeCombinationTableError,
  REMIT_835_BUILD_ERROR_CODES,
  REMIT_835_LOOP_1000A,
  REMIT_835_LOOP_1000B,
  REMIT_835_LOOP_2000,
  REMIT_835_LOOP_2100,
  REMIT_835_LOOP_2110,
  Remit835BuildError,
  build835,
  checkClaimBalance,
  checkCoreCodeCombination,
  checkRemitTotalBalance,
  checkServiceLineBalance,
  get835,
  type CoreBusinessScenario,
  type CoreCodeCombinationErrorCode,
  type CoreCodeCombinationOutcome,
  type CoreCodeCombinationQuery,
  type CoreCodeCombinationResult,
  type CoreCodeCombinationRow,
  type CoreCodeCombinationTable,
  type CoreCodeCombinationTableField,
  type CoreCodeCombinationUnevaluatedReason,
  type Build835AddressSpec,
  type Build835AdjustmentSpec,
  type Build835AmountSpec,
  type Build835ClaimSpec,
  type Build835ContactSpec,
  type Build835EnvelopeSpec,
  type Build835PartySpec,
  type Build835PaymentSpec,
  type Build835PersonSpec,
  type Build835ProviderAdjustmentSpec,
  type Build835ProviderSpec,
  type Build835ReferenceSpec,
  type Build835RemarkSpec,
  type Build835ServiceLineSpec,
  type Build835Spec,
  type Build835TraceSpec,
  type Remit835BuildErrorCode,
  type X12RemitAddress,
  type X12RemitAdjustment,
  type X12RemitAmount,
  type X12RemitClaim,
  type X12RemitContact,
  type X12RemitParty,
  type X12RemitPaymentHeader,
  type X12RemitPerson,
  type X12RemitProvider,
  type X12RemitProviderAdjustment,
  type X12RemitReference,
  type X12RemitRemark,
  type X12RemitServiceLine,
  type X12RemitTrace,
  type X12Remittance,
} from "./transactions/remit/index.js";

// Phase 5 - 837 Healthcare Claim surface (the 005010X222 / X223 / X224 TR3
// families and their published errata).
export {
  CLAIM_837D_LOOP_2000A,
  CLAIM_837D_LOOP_2300,
  CLAIM_837D_LOOP_2400,
  CLAIM_837I_LOOP_2000A,
  CLAIM_837I_LOOP_2300,
  CLAIM_837I_LOOP_2400,
  CLAIM_837P_LOOP_2000A,
  CLAIM_837P_LOOP_2300,
  CLAIM_837P_LOOP_2400,
  CLAIM_837P_LOOP_2410,
  CLAIM_837_LOOP_1000A,
  CLAIM_837_LOOP_1000B,
  CLAIM_837_LOOP_2010AA,
  CLAIM_837_LOOP_2010BA,
  CLAIM_837_LOOP_2010BB,
  CLAIM_837_LOOP_2010CA,
  CLAIM_837_LOOP_2430,
  CLAIM_837_BUILD_ERROR_CODES,
  Claim837BuildError,
  HL_LEVEL_CODES,
  NM1_QUALIFIERS,
  build837D,
  build837I,
  build837P,
  get837Claims,
  type Build837AddressSpec,
  type Build837AdjudicationSpec,
  type Build837AdjustmentSpec,
  type Build837AmountSpec,
  type Build837BillingProviderSpec,
  type Build837ClaimSpec,
  type Build837ContactSpec,
  type Build837DateSpec,
  type Build837DrugSpec,
  type Build837EntitySpec,
  type Build837EnvelopeSpec,
  type Build837HiCodeSpec,
  type Build837NoteSpec,
  type Build837OtherSubscriberSpec,
  type Build837PatientSpec,
  type Build837ReferenceSpec,
  type Build837ServiceLineBaseSpec,
  type Build837ServiceLineDentalSpec,
  type Build837ServiceLineInstitutionalSpec,
  type Build837ServiceLineProfessionalSpec,
  type Build837ServiceLineSpec,
  type Build837Spec,
  type Build837SubscriberInfoSpec,
  type Build837SubscriberSpec,
  type Build837ToothSpec,
  type Claim837BuildErrorCode,
  type X12Claim,
  type X12Claim837Variant,
  type X12ClaimAddress,
  type X12ClaimAmount,
  type X12ClaimContact,
  type X12ClaimDate,
  type X12ClaimEntity,
  type X12ClaimHiCode,
  type X12ClaimMember,
  type X12ClaimNote,
  type X12ClaimReference,
  type X12HierarchicalLevel,
  type X12LineAdjudication,
  type X12LineDrug,
  type X12OtherSubscriber,
  type X12SubscriberInfo,
  type X12ToothInformation,
  type X12_837ServiceLine,
  type X12_837ServiceLineDental,
  type X12_837ServiceLineInstitutional,
  type X12_837ServiceLineProfessional,
  type X12_837Submission,
} from "./transactions/claim/index.js";

// 270 Health Care Eligibility Benefit Inquiry surface (TR3 005010X279A1) -
// the request half of the same pair as the 271 below.
export {
  ELIGIBILITY_270_BUILD_ERROR_CODES,
  Eligibility270BuildError,
  INQUIRY_270_LOOP_2000A,
  INQUIRY_270_LOOP_2000B,
  INQUIRY_270_LOOP_2000C,
  INQUIRY_270_LOOP_2000D,
  INQUIRY_270_LOOP_2100C,
  INQUIRY_270_LOOP_2100D,
  INQUIRY_270_LOOP_2110,
  build270,
  get270Inquiry,
  parse270Inquiries,
  type Build270AddressSpec,
  type Build270DateSpec,
  type Build270DependentSpec,
  type Build270EnvelopeSpec,
  type Build270HeaderSpec,
  type Build270InformationReceiverSpec,
  type Build270InformationSourceSpec,
  type Build270InquirySpec,
  type Build270NameSpec,
  type Build270ProcedureSpec,
  type Build270ReferenceSpec,
  type Build270ServiceTypeSpec,
  type Build270Spec,
  type Build270SubscriberSpec,
  type Build270TraceSpec,
  type Eligibility270BuildErrorCode,
  type X12Inquiry,
  type X12InquiryAddress,
  type X12InquiryDate,
  type X12InquiryDependent,
  type X12InquiryHeader,
  type X12InquiryName,
  type X12InquiryProcedure,
  type X12InquiryReceiver,
  type X12InquiryReference,
  type X12InquiryRequest,
  type X12InquiryServiceType,
  type X12InquirySource,
  type X12InquirySubscriber,
  type X12InquiryTrace,
} from "./transactions/eligibility/index.js";

// Phase 6 - 271 Health Care Eligibility Benefit Response surface (TR3 005010X279A1).
export {
  AAA_CONDITION_LEVELS,
  ELIGIBILITY_271_BUILD_ERROR_CODES,
  ELIGIBILITY_271_LOOP_2000A,
  ELIGIBILITY_271_LOOP_2000B,
  ELIGIBILITY_271_LOOP_2000C,
  ELIGIBILITY_271_LOOP_2000D,
  ELIGIBILITY_271_LOOP_2100C,
  ELIGIBILITY_271_LOOP_2100D,
  ELIGIBILITY_271_LOOP_2110,
  Eligibility271BuildError,
  build271,
  get271Eligibility,
  type Build271AddressSpec,
  type Build271BenefitSpec,
  type Build271DateSpec,
  type Build271DependentSpec,
  type Build271EntitySpec,
  type Build271EnvelopeSpec,
  type Build271InformationReceiverSpec,
  type Build271InformationSourceSpec,
  type Build271MemberSpec,
  type Build271ReferenceSpec,
  type Build271ServiceTypeSpec,
  type Build271Spec,
  type Build271SubscriberSpec,
  type Build271TraceSpec,
  type Eligibility271BuildErrorCode,
  type X12AaaCode,
  type X12AaaCondition,
  type X12AaaConditionKey,
  type X12AaaConditionLevel,
  type X12Eligibility,
  type X12EligibilityAddress,
  type X12EligibilityBenefit,
  type X12EligibilityDate,
  type X12EligibilityDependent,
  type X12EligibilityEntity,
  type X12EligibilityMember,
  type X12EligibilityReference,
  type X12EligibilityServiceType,
  type X12EligibilitySubscriber,
  type X12EligibilityTrace,
} from "./transactions/eligibility/index.js";

// 276 Health Care Claim Status Request surface (TR3 005010X212) - the request
// half of the same pair as the 277 below.
export {
  CLAIM_STATUS_276_BUILD_ERROR_CODES,
  ClaimStatus276BuildError,
  STATUS_276_LOOP_2000A,
  STATUS_276_LOOP_2000B,
  STATUS_276_LOOP_2000C,
  STATUS_276_LOOP_2000D,
  STATUS_276_LOOP_2000E,
  STATUS_276_LOOP_2200,
  STATUS_276_LOOP_2210,
  build276,
  get276StatusInquiry,
  parse276StatusInquiries,
  type Build276AmountSpec,
  type Build276ClaimSpec,
  type Build276DateSpec,
  type Build276DependentSpec,
  type Build276EnvelopeSpec,
  type Build276HeaderSpec,
  type Build276InformationReceiverSpec,
  type Build276InformationSourceSpec,
  type Build276NameSpec,
  type Build276ProcedureSpec,
  type Build276ProviderSpec,
  type Build276ReferenceSpec,
  type Build276ServiceLineSpec,
  type Build276Spec,
  type Build276SubscriberSpec,
  type Build276TraceSpec,
  type ClaimStatus276BuildErrorCode,
  type X12StatusInquiry,
  type X12StatusInquiryAmount,
  type X12StatusInquiryClaim,
  type X12StatusInquiryDate,
  type X12StatusInquiryDependent,
  type X12StatusInquiryHeader,
  type X12StatusInquiryName,
  type X12StatusInquiryProcedure,
  type X12StatusInquiryProvider,
  type X12StatusInquiryReceiver,
  type X12StatusInquiryReference,
  type X12StatusInquiryServiceLine,
  type X12StatusInquirySource,
  type X12StatusInquirySubscriber,
  type X12StatusInquiryTrace,
} from "./transactions/status/index.js";

// Phase 6 - 277 / 277CA Claim Status surface (TR3s 005010X212 / X214).
export {
  CLAIM_STATUS_277_BUILD_ERROR_CODES,
  ClaimStatus277BuildError,
  STATUS_277_LOOP_2000A,
  STATUS_277_LOOP_2000B,
  STATUS_277_LOOP_2000C,
  STATUS_277_LOOP_2000D,
  STATUS_277_LOOP_2000E,
  STATUS_277_LOOP_2200,
  STATUS_277_LOOP_2220,
  build277,
  build277CA,
  get277CADisposition,
  get277Status,
  type Build277ClaimSpec,
  type Build277DateSpec,
  type Build277DependentSpec,
  type Build277EntitySpec,
  type Build277EnvelopeSpec,
  type Build277InformationReceiverSpec,
  type Build277InformationSourceSpec,
  type Build277MemberSpec,
  type Build277ProviderSpec,
  type Build277ReferenceSpec,
  type Build277ServiceLineSpec,
  type Build277Spec,
  type Build277StatusCodeSpec,
  type Build277StatusSpec,
  type Build277SubscriberSpec,
  type Build277TraceSpec,
  type ClaimStatus277BuildErrorCode,
  type X12ClaimStatus,
  type X12ClaimStatusResponse,
  type X12ServiceLineStatus,
  type X12StatusCode,
  type X12StatusDate,
  type X12StatusEntity,
  type X12StatusInfo,
  type X12StatusMember,
  type X12StatusReference,
  type X12StatusTrace,
} from "./transactions/status/index.js";

// Claims attachments: the 277 request for additional information (TR3
// 006020X313) and the 275 that answers it (TR3 006020X314), each read and
// built, each declaring its guide in GS-08 and ST-03.
export {
  ATTACHMENT_275_BUILD_ERROR_CODES,
  Attachment275BuildError,
  RFAI_277_BUILD_ERROR_CODES,
  RFAI_277_LOOP_1000,
  RFAI_277_LOOP_2000,
  RFAI_277_LOOP_2100,
  RFAI_277_LOOP_2200,
  RFAI_277_LOOP_2220,
  Rfai277BuildError,
  X12AttachmentData,
  build275,
  build277RequestForAdditionalInformation,
  get275Attachments,
  get277RequestForAdditionalInformation,
  type Attachment275BuildErrorCode,
  type Build275AttachmentSpec,
  type Build275BeginningSpec,
  type Build275EntitySpec,
  type Build275EnvelopeSpec,
  type Build275LineSpec,
  type Build275ReferenceSpec,
  type Build275Spec,
  type Build275StatusCodeSpec,
  type Build275StatusSpec,
  type Build275TraceSpec,
  type Build277RfaiAmountSpec,
  type Build277RfaiDateSpec,
  type Build277RfaiEntitySpec,
  type Build277RfaiEnvelopeSpec,
  type Build277RfaiHeaderSpec,
  type Build277RfaiLevelSpec,
  type Build277RfaiQuantitySpec,
  type Build277RfaiReferenceSpec,
  type Build277RfaiRequestSpec,
  type Build277RfaiServiceLineSpec,
  type Build277RfaiServiceSpec,
  type Build277RfaiSpec,
  type Build277RfaiStatusCodeSpec,
  type Build277RfaiStatusSpec,
  type Build277RfaiTraceSpec,
  type Rfai277BuildErrorCode,
  type X12AdditionalInformationRequest,
  type X12AdditionalInformationRequestAmount,
  type X12AdditionalInformationRequestClaim,
  type X12AdditionalInformationRequestDate,
  type X12AdditionalInformationRequestEntity,
  type X12AdditionalInformationRequestHeader,
  type X12AdditionalInformationRequestLevel,
  type X12AdditionalInformationRequestQuantity,
  type X12AdditionalInformationRequestReference,
  type X12AdditionalInformationRequestServiceLine,
  type X12AdditionalInformationRequestStatus,
  type X12AdditionalInformationRequestStatusCode,
  type X12AdditionalInformationRequestTrace,
  type X12Attachment,
  type X12AttachmentBeginning,
  type X12AttachmentEntity,
  type X12AttachmentLine,
  type X12AttachmentReference,
  type X12AttachmentStatus,
  type X12AttachmentStatusCode,
  type X12AttachmentSubmission,
  type X12AttachmentTrace,
} from "./transactions/attachments/index.js";

// Phase 7 - 820 Premium Payment surface (TR3 005010X218).
export {
  PREMIUM_820_LOOP_1000A,
  PREMIUM_820_LOOP_1000B,
  PREMIUM_820_LOOP_2000A,
  PREMIUM_820_LOOP_2100A,
  PREMIUM_820_LOOP_2300A,
  PREMIUM_820_LOOP_2310A,
  PREMIUM_820_BUILD_ERROR_CODES,
  Premium820BuildError,
  build820,
  get820Payments,
  type Build820AddressSpec,
  type Build820AdjustmentSpec,
  type Build820DateSpec,
  type Build820EntitySpec,
  type Build820EnvelopeSpec,
  type Build820OpenItemSpec,
  type Build820PartySpec,
  type Build820PaymentSpec,
  type Build820PersonSpec,
  type Build820ReferenceSpec,
  type Build820RemittanceSpec,
  type Build820Spec,
  type Build820TraceSpec,
  type Premium820BuildErrorCode,
  type X12PremiumAddress,
  type X12PremiumAdjustment,
  type X12PremiumDate,
  type X12PremiumEntity,
  type X12PremiumOpenItem,
  type X12PremiumParty,
  type X12PremiumPaymentHeader,
  type X12PremiumPayments,
  type X12PremiumPerson,
  type X12PremiumReference,
  type X12PremiumRemittance,
  type X12PremiumTrace,
} from "./transactions/premium/index.js";

// Phase 7 - 834 Benefit Enrollment and Maintenance surface (TR3 005010X220A1).
export {
  ENROLLMENT_834_LOOP_1000A,
  ENROLLMENT_834_LOOP_1000B,
  ENROLLMENT_834_LOOP_2000,
  ENROLLMENT_834_LOOP_2100A,
  ENROLLMENT_834_LOOP_2300,
  ENROLLMENT_834_LOOP_2320,
  ENROLLMENT_834_BUILD_ERROR_CODES,
  Enrollment834BuildError,
  build834,
  get834Enrollments,
  get834Header,
  type Build834AddressSpec,
  type Build834AmountSpec,
  type Build834CoordinationOfBenefitsSpec,
  type Build834CoverageSpec,
  type Build834DateSpec,
  type Build834EnvelopeSpec,
  type Build834HeaderSpec,
  type Build834MemberNameSpec,
  type Build834MemberSpec,
  type Build834PartySpec,
  type Build834ReferenceSpec,
  type Build834Spec,
  type Enrollment834BuildErrorCode,
  type X12CoordinationOfBenefits,
  type X12Enrollment,
  type X12EnrollmentAddress,
  type X12EnrollmentAmount,
  type X12EnrollmentDate,
  type X12EnrollmentHeader,
  type X12EnrollmentMember,
  type X12EnrollmentParty,
  type X12EnrollmentReference,
  type X12HealthCoverage,
} from "./transactions/enrollment/index.js";

// Phase 7 - 278 Health Care Services Review surface (TR3 005010X217, which
// covers both the request and the response direction). Both builders write
// that identifier into GS-08 and ST-03.
export {
  AUTH_278_BUILD_ERROR_CODES,
  AUTH_278_LOOP_2000A,
  AUTH_278_LOOP_2000B,
  AUTH_278_LOOP_2000C,
  AUTH_278_LOOP_2000D,
  AUTH_278_LOOP_2000E,
  AUTH_278_LOOP_2000F,
  ServicesReview278BuildError,
  build278Request,
  build278Response,
  get278Request,
  get278Response,
  type Build278DateSpec,
  type Build278DecisionSpec,
  type Build278DependentSpec,
  type Build278DiagnosisSpec,
  type Build278EntitySpec,
  type Build278EnvelopeSpec,
  type Build278HeaderSpec,
  type Build278MemberSpec,
  type Build278ReferenceSpec,
  type Build278ReviewSpec,
  type Build278Spec,
  type Build278SubscriberSpec,
  type Build278TraceSpec,
  type ServicesReview278BuildErrorCode,
  type X12AuthDate,
  type X12AuthDiagnosis,
  type X12AuthEntity,
  type X12AuthHeader,
  type X12AuthMember,
  type X12AuthReference,
  type X12AuthTrace,
  type X12ReviewDecision,
  type X12ServiceReview,
  type X12ServicesReview,
} from "./transactions/auth/index.js";

// Phase 6 - shared HL hierarchy primitive (271 / 277 / 277CA walkers).
export type { X12Hl } from "./transactions/shared/hl.js";

// The shared date conversion surface, over every `X12*Date` carrier the typed
// readers surface. `toDate` returns `undefined` unless the caller states the
// zone: an X12 date element is a calendar day, and turning one into an instant
// without a stated offset would mean reading the host machine's timezone.
export { toDate, toISO, toObject } from "./transactions/shared/date-conversion.js";
export type {
  DateParts,
  ToDateOptions,
  X12DateValue,
} from "./transactions/shared/date-conversion.js";

// Phase 3 - acknowledgments surface: parse / build 999 (005010X231A1) and
// envelope-level TA1 as pure functions. See `src/transactions/ack/index.ts`
// for the full barrel.
export {
  ACK_BUILD_ERROR_CODES,
  AckBuildError,
  IK3_SYNTAX_ERROR_CODES,
  IK4_SYNTAX_ERROR_CODES,
  TA1_ACK_CODES,
  TA1_NOTE_CODES,
  X12_ACK_DISPOSITION_CODES,
  build999,
  buildTA1,
  isAcceptDisposition,
  parse999,
  parseTA1,
  type AckBuildErrorCode,
  type Build999ElementErrorSpec,
  type Build999EnvelopeSpec,
  type Build999FunctionalGroupSpec,
  type Build999SegmentErrorSpec,
  type Build999Spec,
  type Build999TransactionResponseSpec,
  type BuildTA1Options,
  type BuildTA1Spec,
  type Ik304Code,
  type Ik403Code,
  type Ta1AckCode,
  type Ta1NoteCode,
  type X12Ack999,
  type X12Ack999Ak1,
  type X12Ack999Ak2,
  type X12Ack999Ak9,
  type X12Ack999ElementNote,
  type X12Ack999Ik3,
  type X12Ack999Ik4,
  type X12Ack999Ik4Position,
  type X12Ack999Ik5,
  type X12Ack999SegmentNote,
  type X12Ack999TransactionResponse,
  type X12AckDispositionCode,
  type X12AckTA1,
} from "./transactions/ack/index.js";

// Phase 8 - spec-clean serializer (the emit half of the parser).
export { serializeX12 } from "./serialize/index.js";
export type { SerializeOptions } from "./serialize/index.js";

// Phase 8 - general-purpose, segment-level interchange builder. The
// domain-specific builders (build835, build837P, …) layer on top of this.
export { buildInterchange, X12_BUILD_ERROR_CODES, X12BuildError } from "./builder/index.js";
// The single sanctioned route a caller-supplied value takes into a `build*`
// refusal message, plus its two bounds. Exported so a consumer can assert the
// ceiling rather than take it on trust - the builder-side counterpart to
// `ALL_WARNING_MESSAGES` on the parse side.
export {
  BUILD_REFUSAL_VALUE_MAX_LENGTH,
  BUILD_REFUSAL_VALUE_MAX_RENDERED,
  renderCallerValue,
} from "./builder/index.js";
export type {
  FunctionalGroupSpec,
  InterchangeSpec,
  SegmentSpec,
  TransactionSetSpec,
  X12BuildErrorCode,
} from "./builder/index.js";

// Phase 9 - trading-partner / companion-guide profile system. Built-ins are
// reached via the `profiles` namespace (not top-level), each grounded in a
// real Tier-2 fixture per the locked hard rule.
export {
  defineProfile,
  getDefaultProfile,
  partitionWarnings,
  profiles,
  setDefaultProfile,
  X12ProfileError,
} from "./profiles/index.js";
export type {
  X12ClassifiedQuirk,
  X12Profile,
  X12ProfileConformance,
  X12ProfileConformancePartition,
  X12ProfileDescription,
  X12ProfileEffect,
  X12ProfileQuirk,
  X12ProfileSpec,
  X12WarningPartition,
} from "./profiles/index.js";
