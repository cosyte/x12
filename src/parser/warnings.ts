 ...
 * apart.
 *
 * Read the bound literally. This reports a row dropped because its AMOUNT did
 * not decode. A row dropped because no claim, service line, coverage or
 * remittance was open to attach it to is a different loss and is not on this
 * channel, and an 820 `RMR` is not on it at all: that row is retained with its
 * own amounts left `undefined` rather than dropped. Nothing is fabricated to
 * stand in and the segments stay verbatim on the transaction set.
 *
 * @example
 * ```ts
