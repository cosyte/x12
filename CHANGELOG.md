...
- **🩺 `buildTA1` now release-escapes its five caller-supplied elements, so an Accept acknowledgment
  ...
  **Three values in the escaped set ever shifted an element; the rest of the set gets longer bytes
  ...
  a `:` and a `^` were emitted verbatim before and are released now, so a control number of
  `"0000:0001"` reads back as `"0000?:0001"`. They are released anyway because the alternative is an
  escaper that is a subset of `escapeRelease`, which would put `buildTA1` back outside the
  type-checking chokepoint this fix routes it through. If you were escaping the value yourself, as
  `KNOWN-LIMITATIONS.md` advised while this was open, drop that - you are now escaping twice
  (`"00000001??"` in, `TA1*00000001????*…` out).
