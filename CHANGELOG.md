...
- **🩺 An EMPTY control number is now REFUSED on emit, where ISA-13 / IEA-02 used to be FABRICATED
  ...
  **The class is wider than that slot, and it has two mechanisms rather than one.** The group and
  ...
  `""`, so an empty one went out as an EMPTY required element at **both** ends of its pair, which
  means each pair still reconciled against itself and `X12_CONTROL_NUMBER_MISMATCH` did not fire
  either. Measured through `buildInterchange`, one variable at a time:
...
required control number and this builder never invents one, so nothing is emitted.` **No new error
  code is minted and no warning code moves**, so nothing a consumer branches on changes. What changes
  is that a call which used to hand you a document now throws.
  ...
  **What it costs, and it is a real break:** a caller relying on `""` to mean "pad me a control
  ...
  - **🛑 The guard is byte-strict `=== ""` and does NOT trim.** A whitespace-only control number is
    still accepted and still padded: `" "` emits ISA-13 as `00000000 `, and `buildTA1` emits
    `TA1*   *…`. Trimming would be a normalisation rule, no source consulted for this package states
    one, and the five in-package guards this mirrors are byte-strict for the same reason. Pinned as a
    test and disclosed in `KNOWN-LIMITATIONS.md` as the one shape still worth screening at your own
    boundary.
  - **It does NOT type-check, and nothing about a non-string changed.** A number or `undefined`
    behaves exactly as it did: through the escaper it draws the type refusal, and at the ISA slots
    `padControl` still throws the typed refusal whose text misleadingly says "exceeds the 9-char spec
    limit". That wart is disclosed elsewhere and is untouched here.
  ...
  - **Every guard sits at the envelope-assembly site, so every guard that already ran keeps its
    precedence.** A spec wrong in two ways reports the same first refusal it reported before:
    `build835`'s balance equation, `build999`'s AK9 count reconciliation and `buildTA1`'s
    accept-must-mean-accept check all still fire ahead of this one.
