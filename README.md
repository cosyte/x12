...
- **Money is exact, and never invented**: every monetary/percent/quantity field decodes as `X12Decimal` (string-backed, BigInt arithmetic), and the library **never `parseFloat`s** an EDI amount. Every one of those slots is `X12Decimal | undefined`: where this library decoded no value from an element it reads `undefined`, so "the sender stated zero" and "the sender stated nothing" are different readings. Through `0.0.12` they were the same `0`.
