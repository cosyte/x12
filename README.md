![@cosyte/x12: parse real-world, vendor-quirky ASC X12 healthcare EDI without reading a TR3](https://cosyte.com/social/cosyte-banner-x12-1200x300.png)
...
- **PHI-disciplined**: synthetic-only fixtures, a PHI commit-gate, warning messages that carry a code and a position but never patient data, and builder refusals that carry structural locators and numeric totals only. **The one deliberate exception:** `X12ParseError.snippet` is a bounded (≤ 64 character) copy of the offending input, so on real traffic it can carry PHI. The library does not redact it. Redact at your call site, or log `err.code` and `err.position` instead.
